      ...
      while (*s && !isspace(*s)) {
        ...
        case 'O':
            ...
            t= qh_skipfilename(s);
            {
              ...
              qh_copyfilename(filename, (int)sizeof(filename), s, (int)(t-s));  /* WARN64 */
              s= t;
              // (To pass CRAN check) if (!freopen(filename, "w", stdout)) {
              // (To pass CRAN check) qh_fprintf(qh ferr, 6044, "qhull error: could not open file \"%s\".", filename);
              // (To pass CRAN check) qh_errexit(qh_ERRinput, NULL, NULL);
