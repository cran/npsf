...
{     ENV *env = tls_get_ptr();
      ...
      {  /* not initialized yet; perform initialization */
         ...
         {  /* initialization failed; display an error message */
            // (To pass CRAN check) fprintf(stderr, "GLPK initialization failed\n");
            // (To pass CRAN check) fflush(stderr);
            /* and abnormally terminate the program */
            // (To pass CRAN check) abort();
         }
      ...
      if (env->self != env)
      {  
        // (To pass CRAN check) fprintf(stderr, "Invalid GLPK environment\n");
        // (To pass CRAN check) fflush(stderr);
        // (To pass CRAN check) abort();
      }
      return env;
