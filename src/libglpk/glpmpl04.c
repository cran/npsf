...
{     xassert(mpl->out_fp == NULL);
      if (file == NULL)
      {
        // (To pass CRAN check) file = "<stdout>";
        // (To pass CRAN check) mpl->out_fp = (void *)stdout;
      }
      else
...
{     xassert(mpl->out_fp != NULL);
  // (To pass CRAN check) if (mpl->out_fp == (void *)stdout)
  // (To pass CRAN check) xprintf("%c", c);
  // (To pass CRAN check) else
...
{     if (setjmp(mpl->jump)) xassert(mpl != mpl);
      ...
      // (To pass CRAN check) if (mpl->out_fp != NULL && mpl->out_fp != (void *)stdout)
      if (mpl->out_fp != NULL) // (To pass CRAN check) 
